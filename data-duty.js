...
function dutyMembers(){ return TEAM.filter(t => t.duty && !t.off).map(t => t.name); }
...
/* 당직 기간 밖에 나간 AS 방문 — 「7월 당직현황」 표의 AS 내역 그대로.
   ...
   tm 은 방문 시각입니다 — 주당직자가 야간에 나간 건은 당직비를 줘야 해서
   시간이 판단 근거가 됩니다 (김기홍 팀장 요청 2026-08-02 4차).
   옛 기록에는 시각이 남아 있지 않아 비워 뒀습니다 — 화면에서 채워 넣을 수 있습니다. */
let DUTY_AS = [
  { d:'2026-07-04', who:'김두혁', tm:'', t:'스마일유통 — 긴급 포스용지 50롤 직배', pay:false },
  { d:'2026-07-04', who:'김두혁', tm:'', t:'디데이산타 김포본점 — 포스용지 50롤 직배', pay:false },
  { d:'2026-07-17', who:'김두혁', tm:'', t:'반땅구리 — 판매/서버 PC교체 · SQL2000 설치 · DB이관', pay:false },
  { d:'2026-07-19', who:'김두혁', tm:'19:00', t:'자이화장품(구월동) — 중고일체형 포스 교체 · SQL2000 설치 · DB이관 · 결제테스트',
    pay:true, why:'자이화장품 오후 7시 방문으로 인해 지급필요' },
  { d:'2026-07-25', who:'임영준', tm:'', t:'행촌칼국수 — 포스 전원불량 아답터 교체 완료', pay:false }
];

/* 야간 방문 판단 — 18시 이후이거나 08시 전이면 야간으로 봅니다.
   주당직자라도 야간 외근은 당직비 지급 대상이라 화면에서 눈에 띄게 표시합니다. */
function isNightTime(tm){
  if(!/^\d{1,2}:\d{2}/.test(tm || '')) return false;
  const h = Number(String(tm).split(':')[0]);
  return h >= 18 || h < 8;
}
...
/* 주당직 한 주기를 날짜로 — 금요일에 시작해 6일 뒤 목요일에 끝납니다.
   경비 표에서 "1주 · 2주" 대신 "7/3~7/9" 로 보여줍니다
   (김기홍 팀장 요청 2026-08-02 4차: "1주 2주 보다 날짜가 나오는 게 좋습니다") */
function dutyWeekRange(fri){
  const a = new Date(fri + 'T00:00:00');
