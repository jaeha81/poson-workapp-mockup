...
/* 지금은 4차입니다. 회차가 바뀌면 이 숫자를 올리세요 —
   팀장님 브라우저에 남아 있던 지난 회차 답이 자동으로 비워집니다. */
const CONFIRM_ROUND = 4;
...
const CONFIRM_ITEMS = [
  ...
  { sec:'3차 반영 (13건)',
    desc:'2026-08-02 저녁에 주신 2차 답변을 전부 반영한 것입니다. 다시 확인하지 않으셔도 됩니다.',
    done:[
  ...
  { sec:'4차 반영 (8건) — 이번에 새로 넣은 것',
    desc:'2026-08-02 밤에 주신 3차 답변을 전부 반영했습니다. 가장 급하다고 고르신 「엑셀 내려받기」부터 넣었습니다. 아래 질문은 이것들이 실제와 맞는지만 봐주시면 됩니다.',
    done:[
      { t:'엑셀 내려받기 — 경비 정산 · 업무량 집계 · 재고 현황', w:'경비 정산',
        d:'가장 급한 것으로 골라주신 것입니다. 「엑셀 내려받기」를 누르면 화면에 보이는 표가 그대로 파일로 저장됩니다. 엑셀에서 바로 열리고 한글도 깨지지 않습니다. 경비는 인원별 지급액 · 업무 경비 내역 · 지급 특이사항까지 한 파일에 담습니다.' },
      { t:'주당직을 「1주 · 2주」 대신 날짜로', w:'경비 정산',
        d:'인원별 지급액 표의 주당직 칸에 「7/3~7/9」처럼 그 주기의 날짜가 나옵니다. 두 주기면 두 줄로 보이고, 아래에 주기 수와 금액을 함께 적었습니다.' },
      { t:'AS 외근 내역에 시각 · 업무 등록에 시각 칸', w:'업무 등록',
        d:'업무 등록에 「업무일 · 시각」 칸을 넣고, 그 아래 「AS 외근으로 함께 등록」에 체크하면 경비 정산의 AS 외근 내역으로 시각과 함께 올라갑니다. 18시 이후 · 8시 전이면 <b>야간</b>으로 표시하고 지급 대상으로 올립니다. 지난 내역은 「시각 입력」을 눌러 채우실 수 있습니다.' },
      { t:'경비 표의 당직자별 행 구분선', w:'경비 정산',
        d:'줄 사이 선을 두껍게 하고, 당직자 이름 칸에 남색 세로선을 넣어 사람마다 확실히 갈라 보이게 했습니다.' },
      { t:'입고 등록에 품목 검색', w:'입고 등록',
        d:'품목 칸에 이름 일부를 치면 목록이 좁혀집니다. 아무것도 치지 않고 목록에서 고르는 것도 그대로 됩니다.' },
      { t:'재고 실사를 엑셀로 한 번에', w:'재고 실사',
        d:'「실사 양식 내려받기」로 지금 수량이 담긴 파일을 받아 세어본 수량만 고쳐 올리면 됩니다. 엑셀에서 복사해 붙여넣어도 되고, 등록된 수량과 같은 줄은 건너뜁니다.' },
      { t:'직원 삭제 · 미사용(퇴사) 처리', w:'직원 · 권한',
        d:'퇴사자는 「미사용 처리」를 쓰시면 로그인과 담당자 선택에서 빠지고 지난 기록은 그대로 남습니다. 아주 지우는 「삭제」도 있지만 되돌릴 수 없어 확인을 한 번 받습니다.' },
      { t:'계정 만들 때 입사일', w:'직원 · 권한',
        d:'계정 만들기에 입사일 칸을 넣었습니다. 이미 있는 직원은 목록의 「입사일」 버튼으로 채우시면 됩니다.' }
    ] },

  { sec:'4차 — 새로 만든 것이 실제와 맞습니까',
    desc:'이번에 넣은 것들이 실제 업무와 맞는지만 봐주시면 됩니다. 모르시면 비워두셔도 됩니다.' },

  { id:'excel', q:'엑셀 내려받기가 쓰시기에 맞습니까?',
    sub:'경비 정산 · 업무량 집계 · 재고 현황 세 화면에서 내려받습니다. 파일은 .csv 이고 엑셀에서 바로 열립니다.',
    opts:['맞습니다', '칸을 더 넣어주세요 (아래에 적어주세요)',
          '.xlsx 파일이어야 합니다', '다른 화면에도 필요합니다 (아래에 적어주세요)'] },

  { id:'dutydate', q:'주당직을 날짜로 바꿨습니다. 이 방식이 맞습니까?',
    sub:'인원별 지급액 표에 「7/3~7/9」처럼 금요일부터 다음 목요일까지가 나옵니다.',
    opts:['맞습니다', '시작 날짜만 나오면 됩니다', '다르게 보여주세요 (아래에 적어주세요)'] },

  { id:'astime', q:'AS 외근 시각과 야간 판단이 맞습니까?',
    sub:'지금은 18시 이후 · 8시 전을 야간으로 보고 지급 대상으로 올립니다. 업무 등록의 「AS 외근으로 함께 등록」 체크로 올라갑니다.',
    opts:['맞습니다', '야간 기준 시각을 바꿔주세요 (아래에 적어주세요)',
          '주말 · 공휴일도 따로 봐야 합니다', '다르게 처리해주세요 (아래에 적어주세요)'] },

  { id:'audxls', q:'재고 실사 엑셀 양식이 쓰시는 파일과 맞습니까?',
    sub:'양식은 「품목명 · 위치 · 세어본 수량」 세 칸입니다. 위치를 비우면 사무실로 봅니다.',
    opts:['맞습니다', '칸이 더 필요합니다 (아래에 적어주세요)',
          '품목 코드로도 찾게 해주세요', '고칠 곳이 있습니다 (아래에 적어주세요)'] },

  { id:'offuser', q:'퇴사자 처리 방식이 맞습니까?',
    sub:'「미사용 처리」는 로그인과 담당자 선택에서만 빼고 기록은 남깁니다. 「삭제」는 계정을 아주 없앱니다.',
    opts:['맞습니다', '미사용도 경비 정산에는 계속 보여야 합니다',
          '고칠 곳이 있습니다 (아래에 적어주세요)'] },

  { id:'account4', q:'계정 칸이 이제 맞습니까?',
    sub:'지금 받는 것 — 이름 · 직급 · 아이디 · 임시 암호 · 입사일.',
    opts:['맞습니다', '더 필요합니다 (아래에 적어주세요 — 예: 사번 · 연락처)'] },

  { sec:'실제 서버로 넘어가는 이야기',
    desc:'3차에서 「PC와 휴대폰을 같이 써야 한다」고 하셨고, 인원은 아직 정하지 못하셨습니다. 인원만 정해주시면 일정과 비용을 정리해 드리겠습니다.' },

  { id:'server4', q:'실제로 쓰실 인원이 정해졌습니까?',
    sub:'기기는 PC와 휴대폰 둘 다로 잡아두겠습니다. 인원만 알려주시면 됩니다.',
    opts:['지금 5명 그대로', '늘어납니다 (아래에 인원을 적어주세요)',
          'AS부서 밖에서도 씁니다 (아래에 적어주세요)', '아직 정하지 못했습니다'] },

  { id:'priority4', q:'서버 붙이기 전에 목업에서 더 볼 것이 있습니까?',
    opts:['없습니다 — 서버 작업으로 넘어가시죠',
          '알림 (당직 · 오더 · 긴급 업무를 휴대폰으로)',
          '사진 첨부 (현장 사진 · 영수증)',
          '휴대폰 화면 최적화',
          '업무량 집계를 등록된 업무에서 자동 계산',
          '그 밖 (아래에 적어주세요)'] },

  { id:'free4', q:'그 밖에 하고 싶은 말씀을 자유롭게 적어주세요.', opts:[] }
];
